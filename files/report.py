...
datasets/report.py
==================
Builds the final Integrated MITM Risk Report that combines:
  • Config validation results  (from existing pipeline)
  • Dataset flow analysis      (DatasetAnalysisResult)
  • LLM threat interpretation  (LLMThreatReport)
"""
...
class IntegratedDatasetReport:
    """
    Merged output of config security + real-dataset MITM analysis + LLM threat intel.
    """
    run_id:           str = ""
    timestamp:        str = ""

    # Config side (from existing ValidationResult)
    config_target:    str   = ""
    config_risk:      float = 0.0
    config_violations: list = field(default_factory=list)

    # Dataset side (from DatasetAnalysisResult)
    dataset_source:       str   = ""
...
class IntegratedReportBuilder:
    """
    Assembles the IntegratedDatasetReport from all pipeline outputs.
    """

    def build(
        self,
        run_id: str,
        config_result=None,       # ValidationResult | None
        config_target: str = "",
        analysis_result=None,     # DatasetAnalysisResult
        llm_report=None,          # LLMThreatReport
    ) -> IntegratedDatasetReport:

        report = IntegratedDatasetReport(
            run_id    = run_id,
        ...
        report.combined_risk = (
            config_w  * report.config_risk +
            dataset_w * report.dataset_risk
        )
