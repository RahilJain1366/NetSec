...
========================
Extension to MITMNetworkAnalyzer that consumes NormalizedFlow objects
from the NIDSDatasetLoader instead of live/pcap traffic.

Add to your mitm/__init__.py:
    from .dataset_analyzer import DatasetMITMAnalyzer

Usage
-----
    from mitm.dataset_analyzer import DatasetMITMAnalyzer
    from datasets import NIDSDatasetLoader

    flows   = NIDSDatasetLoader("datasets/unsw_sample.csv").load(max_samples=100)
    analyzer = DatasetMITMAnalyzer()
    result   = analyzer.analyze_dataset_flows(flows)
"""
...
# ── Indicator & Result dataclasses (mirror existing MITMIndicator style) ──────

@dataclass
class DatasetFlowIndicator:
    """A single MITM-relevant indicator extracted from a flow."""
    attack_type:  str
    ...
    is_confirmed: bool         = False  # True when ground-truth label is attack


@dataclass
class DatasetAnalysisResult:
    """Aggregated result from analyzing a dataset of flows."""
    mode:             str = "dataset"
    source:           str = ""
    total_flows:      int = 0
    attack_flows:     int = 0
    ...
    # Evaluation metrics (when ground-truth labels are present)
...
# Maps UNSW-NB15 / CIC-IDS2017 label substrings → MITM attack-type string
_LABEL_TO_ATTACK = {
...
# Flow-level heuristics (applied regardless of label)
_HEURISTIC_CHECKS = [
    # (name, description, severity, predicate)
    (
    ...
    (
        ...
        lambda f: (
            f.protocol == "udp"
            and f.dst_port not in (None, 53, 5353)
            and f.src_bytes > 2000
        ),
    ),
    (
        "PORT_SCAN_HEURISTIC",
...
class DatasetMITMAnalyzer:
    """
    Analyze a list[NormalizedFlow] (from NIDSDatasetLoader) for MITM
    and network-attack indicators.  Combines:
      1. Ground-truth label → attack-type mapping
      2. Flow-level heuristic detection (TTL anomalies, port patterns, etc.)
    """

    def __init__(self, use_heuristics: bool = True):
        self.use_heuristics = use_heuristics

    def analyze_dataset_flows(
        self,
        flows: list,               # List[NormalizedFlow]
        source: str = "dataset",
        ...
        Parameters
        ----------
        flows   : normalized flows from NIDSDatasetLoader.load()
        source  : human-readable name for logging / reports

        Returns
        -------
        DatasetAnalysisResult with indicators, metrics, risk_score
        """
        t_start = time.time()
            ...
            if self.use_heuristics:
                for hname, hdesc, hsev, hpred in _HEURISTIC_CHECKS:
                    try:
                        if hpred(flow):
                            ind = DatasetFlowIndicator(
                                ...
                                evidence     = (
                                    f"src={flow.src_ip}:{flow.src_port} → "
                                    f"dst={flow.dst_ip}:{flow.dst_port} "
                                    f"sttl={flow.src_ttl} dttl={flow.dst_ttl} "
                                    f"dur={flow.duration_secs:.3f}s "
                        ...
                        pass  # skip heuristic error for single flow
        ...
        raw_risk = sum(
            severity_weights.get(ind.severity, 0.3) for ind in indicators
        )
        ...
        logger.info(
            "Dataset analysis complete: %d flows, %d indicators, risk=%.3f",
            len(flows), len(indicators), risk_score,
        )
        return result
